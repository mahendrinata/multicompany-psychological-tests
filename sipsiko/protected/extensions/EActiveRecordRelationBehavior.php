...
//			'onAfterDelete'=>'afterDelete',
                ...
                switch ($relation[0]) { // relation type such as BELONGS_TO, HAS_ONE, HAS_MANY, MANY_MANY
                    ...
                    case CActiveRecord::HAS_ONE:
                        ...
                        Yii::trace(
                            ...
                            ' foreign-key field for relation ' . get_class($this->owner) . '.' . $name, 'system.db.ar.CActiveRecord'
                        );
    ...
    protected function parseManyManyFk($name, $relation) {
        if (!preg_match('/^\s*(.*?)\((.*)\)\s*$/', $relation[2], $matches))
            throw new CDbException(Yii::t('yii', 'The relation "{relation}" in active record class "{class}" is specified with an invalid foreign key. The format of the foreign key must be "joinTable(fk1,fk2,...)".', array('{class}' => get_class($this->owner), '{relation}' => $name)));
        if (($joinTable = $this->owner->dbConnection->schema->getTable($matches[1])) === null)
            throw new CDbException(Yii::t('yii', 'The relation "{relation}" in active record class "{class}" is not specified correctly: the join table "{joinTable}" given in the foreign key cannot be found in the database.', array('{class}' => get_class($this->owner), '{relation}' => $name, '{joinTable}' => $matches[1])));
...
class ECompositeDbCriteria extends CDbCriteria {
    ...
    public function addInCondition($column, $values, $operator = 'AND') {
        if (is_array($column)) {
            return $this->addCondition(
                    $this->createCompositeInCondition($column, $values), $operator
            );
        }
    ...
    public function addNotInCondition($column, $values, $operator = 'AND') {
        if (is_array($column)) {
            return $this->addCondition(
                    'NOT ' . $this->createCompositeInCondition($column, $values), $operator
            );
        }
